export const DONATION_CONFIRMATION_MESSAGE =
  'Thank you. You just moved a cat closer to a couch. Watch your inbox; we love sharing where the help goes.';

export function sponsorshipConfirmationMessage(kittenName) {
  return `You're officially in ${kittenName}'s corner. Thank you. We'll keep you posted on how they're doing. Welcome to the rescue.`;
}

export function sponsorshipOverflowDisclosure(kittenName) {
  return `Sponsorships help cover ${kittenName}'s care. Once ${kittenName} is fully funded or finds a home, additional gifts support kittens just like them. Pawsitive Transformations directs all sponsorship funds where the cats need them most.`;
}

export const SPONSOR_TIERS = [
  { id: 'kickstart', name: 'Kickstart Kitty', price: 15 },
  { id: 'shots', name: 'Shots & Chips', price: 40 },
  { id: 'belly', name: 'Belly & Box', price: 75 },
  { id: 'fix', name: 'The Big Fix', price: 135 },
  { id: 'caboodle', name: 'Whole Kitten Caboodle', price: 350 },
];

export function resolveSponsorTier(amount) {
  const numeric = Number(amount);
  if (!Number.isFinite(numeric)) return 'Custom';
  const match = SPONSOR_TIERS.find((tier) => tier.price === numeric);
  return match?.name || 'Custom';
}
